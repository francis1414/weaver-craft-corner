...
CREATE OR REPLACE FUNCTION public.validate_subscriber_insert()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  ...
  NEW.status := 'subscribed';
  NEW.source := 'footer_newsletter';
  IF length(NEW.email) > 255 OR NEW.email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' THEN
...
DROP TRIGGER IF EXISTS validate_subscriber_insert ON public.subscribers;
CREATE TRIGGER validate_subscriber_insert BEFORE INSERT ON public.subscribers FOR EACH ROW EXECUTE FUNCTION public.validate_subscriber_insert();
CREATE UNIQUE INDEX IF NOT EXISTS subscribers_email_unique_lower_idx ON public.subscribers (lower(email));
DROP POLICY IF EXISTS "Anyone can subscribe" ON public.subscribers;
CREATE POLICY "Visitors can subscribe with valid defaults" ON public.subscribers FOR INSERT TO anon, authenticated WITH CHECK (
  status = 'subscribed' AND source = 'footer_newsletter' AND length(email) <= 255
);
